...
function Section({ title, children, span = 1 }: { title: string; children: React.ReactNode; span?: number }) {
  return (
    <div style={{ gridColumn: `span ${span}`, backgroundColor: C.surface1, border: `1px solid ${C.hairline}`, borderRadius: R.lg, padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 12 }}>
      <p style={{ margin: 0, fontSize: 11, fontWeight: 600, color: C.inkTertiary, textTransform: 'uppercase' as const, letterSpacing: '0.06em', fontFamily: font.text }}>{title}</p>
      {children}
    </div>
  );
}
...
function StatusDistribution({ issues }: { issues: ProfileIssue[] }) {
  ...
  const GROUP_COLOR: Record<string, string> = { todo: C.inkTertiary, doing: C.primary, done: 'transparent', unknown: C.inkTertiary };

  const sorted = Object.entries(counts).sort((a, b) => b[1] - a[1]);
  const max = Math.max(...sorted.map(([, v]) => v), 1);

  if (sorted.length === 0 && issues.some((i) => i.statusGroup !== 'done')) {
       return <p style={{ margin: 0, fontSize: 13, color: C.inkTertiary, fontFamily: font.text }}>No active status breakdown available.</p>;
  } else if (sorted.length === 0) {
    return <p style={{ margin: 0, fontSize: 13, color: C.inkTertiary, fontFamily: font.text }}>No issues found in the analyzed scope.</p>;
  }
      ...
      {sorted.map(([status, count]) => {
        ...
        return (
          <div key={status} style={{ display: 'grid', gridTemplateColumns: '140px 1fr 28px', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 12, color: C.inkSubtle, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const, fontFamily: font.text }}>{status}</span>
            <div style={{ height: 6, borderRadius: 3, backgroundColor: C.canvas, overflow: 'hidden' }}>
              <div style={{ width: `${(count / max) * 100}%`, height: '100%', backgroundColor: color, borderRadius: 3, transition: 'width 0.4s' }} />
            </div>
            <span style={{ fontSize: 12, fontWeight: 700, color: C.inkMuted, textAlign: 'right' as const, fontVariantNumeric: 'tabular-nums' as const, fontFamily: font.text }}>{count}</span>
          </div>
        );
      })}
    </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-around' }}>
        <StatPill value={assigned} label="Assigned" color={C.primary} />
        <div style={{ width: 1, backgroundColor: C.hairline }} />
        <StatPill value={raised}   label="Raised"   color="#a855f7" />
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 11, color: C.inkTertiary, width: 60, fontFamily: font.text }}>Assigned</span>
          <div style={{ flex: 1, height: 6, borderRadius: 3, backgroundColor: C.canvas, overflow: 'hidden' }}>
            <div style={{ width: `${(assigned / total) * 100}%`, height: '100%', backgroundColor: C.primary, borderRadius: 3 }} />
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 11, color: C.inkTertiary, width: 60, fontFamily: font.text }}>Raised</span>
          <div style={{ flex: 1, height: 6, borderRadius: 3, backgroundColor: C.canvas, overflow: 'hidden' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        ...
        <span style={{ fontSize: 12, color: C.inkTertiary, fontFamily: font.text }}>{collab} of {total} tickets</span>
      </div>
      <div style={{ height: 8, borderRadius: 4, backgroundColor: C.canvas, overflow: 'hidden' }}>
        <div style={{ width: `${pct}%`, height: '100%', backgroundColor: '#06b6d4', borderRadius: 4, transition: 'width 0.4s' }} />
      </div>
      <p style={{ margin: 0, fontSize: 12, color: C.inkTertiary, fontFamily: font.text }}>
...
function IssueRow({ issue, badge, badgeColor, onClick }: {
  ...
  return (
    <div
      style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: R.sm, cursor: 'pointer', backgroundColor: hovered ? C.surface2 : 'transparent', borderBottom: `1px solid ${C.hairline}`, margin: '0 -10px' }}
      onClick={onClick}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      <span style={{ fontSize: 11, color: C.inkTertiary, flexShrink: 0, minWidth: 52, fontVariantNumeric: 'tabular-nums' as const, fontFamily: font.mono }}>#{issue.itemNo}</span>
      <span style={{ fontSize: 13, color: C.inkMuted, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const, fontFamily: font.text }}>{issue.title}</span>
      <span style={{ fontSize: 11, color: C.inkTertiary, flexShrink: 0, fontFamily: font.text }}>{issue.sprintName}</span>
      {badge && (
        <span style={{ fontSize: 11, fontWeight: 700, color: badgeColor, padding: '2px 6px', borderRadius: R.pill, border: `1px solid ${badgeColor}44`, backgroundColor: `${badgeColor}11`, flexShrink: 0, fontFamily: font.text }}>
          {badge}
        </span>
      )}
    </div>
...
function SprintHistorySection({ history }: { history: SprintHistoryItem[] }) {
  if (history.length === 0) return <p style={{ margin: 0, fontSize: 13, color: C.inkTertiary, fontFamily: font.text }}>No sprint history yet.</p>;

  const maxAssigned = Math.max(...history.map((h) => h.assigned), 1);

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px 80px 80px 100px', gap: 10, padding: '4px 10px', fontSize: 10, fontWeight: 600, color: C.inkTertiary, textTransform: 'uppercase' as const, letterSpacing: '0.05em', borderBottom: `1px solid ${C.hairline}`, marginBottom: 4, fontFamily: font.text }}>
        <span>Sprint</span>
        <span>Project</span>
      ...
      {history.map((h) => (
        <div key={h.sprintId} style={{ display: 'grid', gridTemplateColumns: '1fr 120px 80px 80px 100px', gap: 10, padding: '8px 10px', borderBottom: `1px solid ${C.hairline}`, alignItems: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            <span style={{ fontSize: 13, color: C.inkMuted, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const, fontFamily: font.text }}>{h.sprintName}</span>
            {h.endDate && <span style={{ fontSize: 11, color: C.inkTertiary, fontFamily: font.text }}>{fmtDate(h.endDate)}</span>}
          </div>
          <span style={{ fontSize: 12, color: C.inkTertiary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const, fontFamily: font.text }}>{h.projectName}</span>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3, alignItems: 'flex-end' }}>
            <span style={{ fontSize: 13, fontWeight: 600, color: C.inkMuted, fontVariantNumeric: 'tabular-nums' as const, fontFamily: font.text }}>{h.assigned}</span>
            <div style={{ width: 60, height: 3, borderRadius: 2, backgroundColor: C.canvas, overflow: 'hidden' }}>
              <div style={{ width: `${(h.assigned / maxAssigned) * 100}%`, height: '100%', backgroundColor: C.primary }} />
            </div>
          </div>
          <span style={{ fontSize: 13, fontWeight: 600, color: C.success, textAlign: 'right' as const, fontVariantNumeric: 'tabular-nums' as const, fontFamily: font.text }}>{h.done}</span>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'flex-end' }}>
            <div style={{ width: 48, height: 6, borderRadius: 3, backgroundColor: C.canvas, overflow: 'hidden' }}>
              <div style={{
                width: `${h.completionPct}%`, height: '100%', borderRadius: 3,
                backgroundColor: h.completionPct >= 80 ? C.success : h.completionPct >= 50 ? C.primary : h.completionPct >= 25 ? C.warning : C.danger,
              }} />
            </div>
            <span style={{ fontSize: 12, fontWeight: 700, color: C.inkMuted, minWidth: 34, textAlign: 'right' as const, fontVariantNumeric: 'tabular-nums' as const, fontFamily: font.text }}>{h.completionPct}%</span>
          </div>
        </div>
      ))}
    </div>
...
export function UserProfilePage() {
  ...
  if (error || !profile) return (
    <div style={s.page}>
      ...
      <p style={{ color: C.danger, fontSize: 14, fontFamily: font.text }}>{error ?? 'Profile not found.'}</p>
    </div>
  );
    ...
    <div style={s.page}>
      ...
      <div style={s.heroCard}>
        ...
        <div style={s.heroInfo}>
          <span style={{ fontSize: 20, fontWeight: 700, color: C.inkMuted, fontFamily: font.display }}>{user.name}</span>
          <span style={{ fontSize: 13, color: C.inkTertiary, fontFamily: font.text }}>{user.email ?? '—'}</span>
          <span style={{ fontSize: 12, fontWeight: 700, padding: '3px 9px', borderRadius: R.pill, border: `1px solid ${roleColor(user.role)}44`, color: roleColor(user.role), backgroundColor: `${roleColor(user.role)}11`, alignSelf: 'flex-start', marginTop: 2, fontFamily: font.text }}>
            {user.role}
          </span>
        </div>
        ...
        <div style={s.heroStats}>
          ...
          <StatPill value={summary.todo}    label="Todo"     color={C.inkTertiary} />
          <StatPill value={summary.doing}   label="In Progress" color={C.primary} />
          <StatPill value={summary.done}    label="Done"     color={C.success} />
          <StatPill value={summary.stale}   label={`Stale ≥${staleDays}d`} color={summary.stale > 0 ? C.warning : C.inkSubtle} />
          <StatPill value={summary.overdue} label="Overdue"  color={summary.overdue > 0 ? C.danger : C.inkSubtle} />
      ...
      <div style={s.grid}>
        ...
        <Section title={`Stale Tickets (≥${staleDays} days)`} span={2}>
          {staleIssues.length === 0
            ? <p style={{ margin: 0, fontSize: 13, color: C.success, fontFamily: font.text }}>No stale tickets</p>
        ...
        <Section title="Overdue Tickets" span={2}>
          {overdueIssues.length === 0
            ? <p style={{ margin: 0, fontSize: 13, color: C.success, fontFamily: font.text }}>No overdue tickets</p>
        ...
        <Section title={`Sprint History (${history.length} sprint${history.length !== 1 ? 's' : ''})`} span={4}>
          {histLoading
            ? <p style={{ margin: 0, fontSize: 13, color: C.inkTertiary, fontFamily: font.text }}>Loading history…</p>
          ...
          <Section title={`Tickets Raised – Last 30 Days (${raisedIssues.length})`} span={4}>
            {raisedIssues.map((issue) => (
              <IssueRow
                ...
                badgeColor={issue.statusGroup === 'done' ? C.success : issue.statusGroup === 'doing' ? C.primary : C.inkTertiary}
...
const s: Record<string, React.CSSProperties> = {
  page:     { minHeight: '100vh', backgroundColor: C.canvas, color: C.inkMuted, fontFamily: font.text, padding: '0 24px 48px' },
  header:   { display: 'flex', alignItems: 'center', gap: 20, padding: '32px 0 40px', borderBottom: `1px solid ${C.hairline}`, marginBottom: 32 },
  ...
  heroCard: { backgroundColor: C.surface1, border: `1px solid ${C.hairline}`, borderRadius: R.lg, padding: '24px 28px', display: 'flex', alignItems: 'center', gap: 24, marginBottom: 28, flexWrap: 'wrap' as const },
  ...
  heroDivider: { width: 1, height: 64, backgroundColor: C.hairline, flexShrink: 0 },
